...
public class PlayerController1 : MonoBehaviour
{
    ...
    public Transform target1; // 近づくべきオブジェクトのTransform
    //敵イベントパネル
    ...
    public Transform targetE1; // 近づくべきオブジェクトのTransform
    private Enemy enemy;
    ...
    void Update()
    {
        ...
        if (!isMoving)
        {
            ...
            if (mineral2 == mineral.go)
            {
                if (Vector3.Distance(transform.position, target1.position) < 2f)
                {
                    mineral2 = mineral.stop;
                    // 近づいたときの処理を記述する
                    panel.SetActive(true);

                }
            }
            else if (mineral2 == mineral.stop)
            {
                if (Vector3.Distance(transform.position, target1.position) > 2f)
                {
                    mineral2 = mineral.go;
            ...
            if (enemy1 == Enemy.go)
            {
                if (Vector3.Distance(transform.position, targetE1.position) < 2f)
                {
                    enemy1 = Enemy.stop;
                    // 近づいたときの処理を記述する
                    panelE.SetActive(true);
            ...
            totalWeight =
                PutItemInBackPack.itemWeight1 * PutItemInBackPack.quantity1 +
